...
{
    if (!epc || epc_len == 0 || epc_len > sizeof(g_tags[0].epc))
        return;
    for (int i = 0; i < g_tag_count; ++i)
    {
        if (g_tags[i].epc_len == epc_len && memcmp(g_tags[i].epc, epc, epc_len) == 0)
        {
            if (rssi > g_tags[i].best_rssi)
                g_tags[i].best_rssi = rssi;
            return;
        }
    }
...
static void tags_print_current(void)
{
    ...
    else
    {
        ...
        {
            printf("[%02d] EPC=", i);
            for (uint16_t j = 0; j < g_tags[i].epc_len; ++j)
                printf("%02X", g_tags[i].epc[j]);
...
static void r2000_handle_frame(const uint8_t *p, uint16_t len)
{
    ...
    if (expected_total != len)
        return;
    if (p[0] != COMM_HEAD || p[len - 1] != COMM_END)
        return;
...
static void rx_task(void *arg)
{
    ...
    while (1)
    {
        ...
        if (n > 0)
            r2000_process_stream(buf, (size_t)n);
